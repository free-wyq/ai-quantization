...
python engine/backtest.py midterm 000001 -p adx_entry_min=15    # 覆盖参数
...
**日期格式注意**:`engine/backtest.py`/`fetcher` 用 `YYYYMMDD`(如 `20250101`),不是 ISO。`run()` 函数签名里 `start_date`/`end_date` 是 `str` 默认 `"20260101"`/`"20260818"`,argparse `--start`/`--end` 默认值在 `engine/backtest.py` main()。
...
strategies/               策略(只产出 entries/exits/indicators,不回测)
engine/backtest.py        回测入口(vectorbt.Portfolio.from_signals + 绩效 + 看板导出)
engine/costs.py           成本模型唯一定义
research/batch_backtest.py  30股批量回测
research/optimize.py      参数网格+样本外验证
```

**关键约束:策略绝不自己跑回测。** `midterm.py` docstring 明确"不重写回测器,本类只负责生成信号"。策略 `run(df)` 返回**固定 5 元组** `(entries, exits, indicators, size, reasons)`(`size` 可 None=等权满仓,`reasons` 可 None)。回测逻辑(成本/资金/止损止盈)统一在 `engine/backtest.py` 的 `vbt.Portfolio.from_signals` 调用里,`research/batch_backtest.py` 和 `research/optimize.py` 各自复刻一份简化版(成本数值已统一 import 自 `engine/costs.py`)。

### 策略基类:模板方法模式

`strategies/base.py` 的 `Strategy.run(df)` 是**模板骨架**(子类一般不重写):组装公共指标 → 调子类 `generate(df)` 钩子 → 返回固定 5 元组。
...
`strategies/__init__.py` 在包 import 时用 `pkgutil.walk_packages` 扫描所有子模块,收集 `Strategy` 子类(排除 `Strategy` 本身,且 `obj.__module__ == modname` 防止重复注册)填入 `STRATS` dict,key 是 `cls.name`。

- 内置策略直接放 `strategies/*.py`(当前仅 `midterm.py`)
- **用户自定义策略放 `strategies/custom/`**(子包不存在则创建),框架自动发现,**无需改任何框架代码**
- 新策略:继承 `Strategy`,设 `name`/`label`/`params`,实现 `generate(df)` 返回 `SignalResult`,indicator values 用 `series_to_list()` 转(`.fillna(False)` 必须做,vectorbt 不吃 NaN)

### A股成本模型(三处必须一致)

`COST_FEES=0.0008`(佣金万3+印花税千1,买卖平均)+ `COST_SLIPPAGE=0.001`(0.1%)+ `INIT_CASH=100000`,**唯一定义在 `engine/costs.py`**。`engine/backtest.py`/`research/batch_backtest.py`/`research/optimize.py` 全部 import 自它。**改成本只改 `engine/costs.py` 一处**。

### 看板数据流(不可改 dashboard.html 的约定)

`framework/results/dashboard.html` 是**手写静态文件,绝不修改**(用户明确诉求,见 `dashboard_server.py` 模块 docstring)。`dashboard_server.py` 在响应 dashboard.html 时,运行时扫描 `framework/results/runs/*.json`,注入 `window.__RUNS__` 列表(replace 掉 `<script src="runs/index.js">`)。所以:

- 每次回测 `engine/backtest.py` 的 `_export_result()` 往 `runs/` 写一个独立 JSON(`{symbol}_{strategy}_{时间戳}.json`),含 candles/buys/sells/equity/indicators/metrics
- 看板刷新即重新扫描,**不重新生成 HTML**
- `file://` 打开无效,必须走 `dashboard_server.py`(8000端口,仓库根执行)
- buys/sells 从 `pf.trades.records_readable` 提取真实成交,不是原始信号(避免无持仓时的虚假卖出)

### 中期复合策略(midterm) + 因子库

`strategies/midterm.py` 是当前唯一内置策略,七层闭环架构见 `DESIGN.md` 第二部分。它依赖 `factors/` 因子库(纯函数:输入日K,输出等长对齐 Series):
...
- ⚠️ 退出逻辑(ATR跟踪止损/量价背离/ADX)**已内联在 midterm.py**(不在 factors/)(`_atr`/`_build_exits` 等),`factors/exit.py` 早已删除。
